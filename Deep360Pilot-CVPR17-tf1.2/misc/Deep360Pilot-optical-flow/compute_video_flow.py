...
EXE_PATH = os.getcwd()#os.path.join(os.getcwd(), 'Deep360Pilot-optical-flow')
NAMEs = sorted(map(lambda x: x.split('/')[-1], glob.glob(DATA_PATH + 'frame_' + DOMAIN + '/*')))
DEBUG = False

def compute_of(path, i):
	cmd = '{}/of {}frame_{:d} {}frame_{:d}'.format(EXE_PATH, path, i, path, i+1)
	call(cmd, shell=True)
...
if __name__ == '__main__':
	...
	for name in NAMEs:
		...
		Parallel(n_jobs=10)(delayed(compute_of)(path, i) for i in range(1, n_frames))
